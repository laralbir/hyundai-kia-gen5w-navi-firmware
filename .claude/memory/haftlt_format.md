...
### `.hafls` — tabla dispersa candidata a índice geográfico (nuevo, mismo día)

Tras agotar las hipótesis de conexión nombre↔registro en `.haftlt`, se investigó `.hafls` (capa pan-EU, 84 MB) desde cero — cabecera con layout distinto al de `.haftlt`, sin la misma tabla de nombres. Se encontró en offset `0x108` una tabla de 464.688 entradas de 8 bytes, **idéntica entre builds** pese a que el resto del fichero cambia casi por completo.

⚠️ Caracterización inicial (solo primeras ~30 entradas) sugería un stride uniforme de 3MB — **corregido tras escanear la tabla completa**: es en realidad una **tabla dispersa** con huecos de relleno masivos (`0xFFFFFFFF`/`0x00000000`) intercalados con entradas reales cuya clave crece monótonamente — patrón compatible con una tabla hash indexada por clave de tile (p.ej. Morton/Z-order), más prometedor que un array denso pero pendiente de aislar los tramos y probar la hipótesis de coordenada con prueba de permutación desde el principio. Detalle completo: [`docs/hafls_tile_table.md`](../../docs/hafls_tile_table.md).
