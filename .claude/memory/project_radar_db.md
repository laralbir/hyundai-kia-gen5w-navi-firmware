...
### 🎯 Continuación misma sesión — renderizado real vía topología, sin coordenadas GPS
...
**Lo que sí funcionó:** usar la adyacencia YA confirmada (`f2`/`f3` de `linked_records` = índice ±1 del propio registro, sesión 2026-07-10) para construir un grafo y dibujarlo con layout por fuerzas (`networkx`), sin ninguna coordenada. Verificado a escala completa en España (23.528 registros): 84,6% de `f2`/`f3` son exactamente `idx∓1` (confirma la hipótesis "índice ±1"), el 15,4% restante son saltos grandes — candidatos a cruces reales. 4.733 componentes conexos, el mayor con 199 nodos; los componentes grandes renderizan como **curvas y bifurcaciones topológicamente coherentes, no ruido** — verificado visualmente. Cruzado contra `SPEED_PATCH.db`: 8,4% de los nodos de los componentes grandes tienen límite de velocidad real confirmado (coherente con la cobertura ~10% ya documentada).

**Es la primera vez en toda la investigación que se renderiza algo real y verificable del grafo de carreteras** — no geográficamente orientado ni a escala, pero topológicamente fiel (como un mapa de metro). Herramienta: `tools/haftlt_graph_viewer/`. Detalle completo: `docs/road_network_topology.md`.
