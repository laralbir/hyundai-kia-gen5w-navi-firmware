...
**Estado tras esta sesión:** tanto `.hafr` como `.hafp03` dan nombre+fonética/tipo con byte de país consistente (`0x2f`=España en ambos), pero **ninguno de los dos da coordenadas** pese a búsquedas sistemáticas con prueba de significancia.

### Continuación misma sesión — `.hafgsi` resuelto: es paginación por bytes, no índice geográfico

`.hafgsi` (274 MB) se decodificó por completo: es una tabla de troceo lineal (stride fijo 12,58 MB, luego 12,58 MB+1, luego 512 MB — varios niveles jerárquicos) que referencia el espacio de direcciones **virtual de los 16 ficheros `.hafp`/`hafp01`-`hafp15` concatenados en orden numérico** — verificado con precisión matemática exacta (cada fichero recibe exactamente `ceil(tamaño/12.582.912)` entradas, sin huecos). Es un índice de acceso/streaming (tipo tabla de páginas), **no geográfico** — recorre cada fichero de principio a fin en bloques de tamaño fijo, sin relación con contenido.

**Balance de la sesión completa:** tres ficheros (`.hafr`, `.hafp03`, `.hafgsi`) recibieron análisis completo y riguroso, verificado matemáticamente en los tres casos — y en los tres, la estructura resultó ser un mecanismo de indexación/conteo genérico, no coordenadas. Primera vez que se agota esta línea de ataque con este nivel de rigor en tres ficheros a la vez. El bloqueo de "encontrar geometría por inspección externa" se mantiene — la vía más prometedora sigue siendo descifrar el parser real (`appnavi`, exploit físico `gen5w`).

### Continuación misma sesión — intento de renderizado vía topología, DESCARTADO tras control; herramienta final es una tabla, no un dibujo

El usuario preguntó si hacía falta la coordenada absoluta o bastaba con valores relativos de nodos/grafo. Se probó primero geometría tipo malla en `.hafp03` (bloques de u16 con 90% de tríos consecutivos compartiendo vértice — señal estadística fuerte) pero **al renderizarlo visualmente resultó ser ruido** (dos contadores creciendo juntos, no una forma real) — descartado tras verificación visual, no solo estadística.

Después se probó usar la adyacencia ya confirmada (`f2`/`f3` = índice ±1) para dibujar un grafo con layout por fuerzas (`networkx`, spring layout) — el resultado (curvas, bucles) **parecía** a primera vista una red de carreteras real. **El usuario cuestionó esa conclusión, correctamente.** Control decisivo: cadenas puramente sintéticas y aleatorias (`nx.path_graph`, sin ningún dato real) con el mismo layout producen **el mismo tipo de garabatos** — es un artefacto estético del algoritmo aplicado a cualquier grafo disperso tipo cadena, no evidencia de nada sobre carreteras. Se retiró por completo esa afirmación.

**Hallazgo adicional al reconstruir la herramienta sin layout:** el grado máximo de cualquiera de los 23.528 registros de España es exactamente **2** (verificado con conjuntos) — `f2`/`f3` son listas enlazadas simples, sin ningún cruce/intersección codificado en ese par de campos. Corrige también la lectura anterior de "15,4% de saltos = candidatos a cruces reales" — esos saltos son simplemente el "siguiente" real de la lista enlazada (no necesariamente el índice físico contiguo), no bifurcaciones.
...
### 🏁 Continuación misma sesión (usuario pide centrarse solo en renderizar el mapa, no en radares) — hipótesis `PackedTileId` también probada, negativa; vía de inspección externa agotada

Se probó exactamente la hipótesis que quedó pendiente arriba: escaneo de valores `u32` alineados que decodifiquen como `PackedTileId` válido (nivel 0-15) con bbox solapando España, en `.hafp03` (5M palabras, niveles 3-11) y en el árbol de `.hafr` (6.776 registros × 9 campos). `.hafr`: 0 candidatos. `.hafp03`: 125 candidatos, pero **prueba de permutación decisiva — los 10 controles con bytes barajados superan TODOS al resultado real** (rango 303-341 vs real 125) — es decir, los datos reales tienen MENOS candidatos que ruido puro, no más. Negativo sin ambigüedad.

**Nota metodológica nueva:** los niveles 12-15 de `PackedTileId` son casi siempre "válidos" para cualquier valor aleatorio (confirmado con control de ruido uniforme) — cualquier prueba futura debe restringirse a niveles bajos/medios o usar permutación, igual que con Morton completo.

**Balance:** con el algoritmo NDS exacto ahora confirmado (no aproximado), se probaron las dos hipótesis de geometría más fundamentadas (Morton completo y `PackedTileId`+rejilla) contra los ficheros más prometedores — ambas negativas con rigor estadístico. **Esto agota, con alta certeza, la vía de "encontrar geometría por inspección externa de bytes"** tal como está planteada hoy. La vía que queda es descifrar `appnavi` (exploit físico `gen5w`, ya preparado y listo — ver `tools/README.md` "Checklist del día D" — pendiente solo de acceso físico al vehículo).
