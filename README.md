...
- [CAN bus / OBD2 — nueva vía de investigación](docs/can_bus_obd2_investigation.md) — evalúa si el exploit del HU permite llegar al bus de diagnóstico del vehículo: los Hyundai/Kia con Central Gateway llevan un Security Gateway (SGW) que exige certificado digital oficial para *coding*/pruebas de actuador/funciones especiales (sin bypass público conocido), mientras que los DTC genéricos (leer/borrar) son OBD2 estándar y no dependen del HU ni del SGW. Documenta tres vías evaluadas (OBD2 directo al DLC, HU rooteado, bypass del SGW) y un plan de acción priorizado por coste/beneficio.
