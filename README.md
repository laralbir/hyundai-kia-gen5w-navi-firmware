...
│   └── can_bus_obd2_investigation.md   Nueva vía: acceso a CAN bus/OBD2 del vehículo — arquitectura CGW/SGW de Hyundai/Kia, qué da cada camino (OBD2 estándar, HU rooteado, bypass SGW), plan de acción
└── tools/                              Herramientas y guías operativas para RE del HU
    ...
    └── gen5w-qemu/                      🎯 Virtualización gráfica del HU con QEMU (alternativa al chroot de gen5w-docker)
        ├── README.md                   Guía completa: qué está confirmado (framebuffer real, Qt5 compila/arranca) vs qué no (UI real del HU, aún sin rootfs descifrado)
        ├── run_graphical.sh            Script todo-en-uno: rootfs → imagen → arranque gráfico en un comando
        ├── fetch_generic_kernel.sh     Extrae vmlinuz+initrd de un linux-image-amd64 de Debian (vía apt en Docker)
        ├── build_rootfs_image.sh       Convierte un rootfs extraído (tar) en imagen ext4 arrancable
        └── boot.sh                     Arranca QEMU con virtio-blk (root real) + virtio-vga (framebuffer real); modo ventana o headless con captura automática
```
...
- [Ecosistema de exploits gen5w](docs/gen5w_exploit_ecosystem.md) — cadena completa para descifrar OTA: exploit USB (`navi_extended`), extracción de `DecryptToPIPE` + `decryption_key.der`, Docker `update_decryptor`, patcher de persistencia y entorno `gen5w-docker`. Incluye validación práctica del pipeline completo con dry-run sintético (sin HU físico): confirma mecánica de los tres Docker, corrige un bug de documentación y confirma arquitectura x86_64 real bajo emulación de Docker en Apple Silicon. 🎯 Además evalúa y valida virtualización completa con QEMU (`tools/gen5w-qemu/`) como alternativa al chroot: kernel real arranca en 2,3s sin KVM, root filesystem real vía `virtio-blk`, y **framebuffer gráfico real confirmado con una captura de pantalla** (`virtio-vga`, consola del kernel a 1280×800) — más cerca de un "emulador" real que el chroot, que no puede dar salida gráfica.
...
- [CAN bus / OBD2 — nueva vía de investigación](docs/can_bus_obd2_investigation.md) — evalúa si el exploit del HU permite llegar al bus de diagnóstico del vehículo. Arquitectura real confirmada por fuente primaria (P-CAN/B-CAN/C-CAN/I-CAN/M-CAN vía Central Gateway, HU en M-CAN junto a amplificador/cuadro/E-CALL) y precedente académico directo con CVE real — **KOFFEE/CVE-2020-8539**: un head unit hermano de Kia inyectaba tramas CAN reales ejecutando `micomd -c inject` desde una simple app Android, sin tocar nunca motor/frenos/DTC (aislados por el Security Gateway). Da un objetivo de grep concreto (`micomd`, `sendMicomMsg`, `automotivefw`) para cuando el rootfs mango/S5W esté descifrado, y documenta la vía OBD2 estándar directa al DLC (sin exploit) para DTCs genéricos. Añade además consumo/autonomía (disponible hoy sin exploit vía Kia Connect y/o pantalla nativa del firmware), batería del sistema mild-hybrid 48V (SOC confirmado llegando al cuadro de instrumentos vía M-CAN, pero no expuesto en la nube), y cámara frontal (es el sensor ADAS de colisión, sin salida de vídeo al HU — solo la trasera de aparcamiento sí lo tiene).
