...
7. **Tabla de nombres de calle (Pascal-strings UTF-8) totalmente decodificada** en los 4 países, con extracción automática vía `tools/haftlt_parser/parse_haftlt.py` — primer texto legible con significado geográfico de toda la investigación. La tabla de 16 bytes que la sigue está estructuralmente confirmada (conteo exacto) aunque el significado de sus campos y su conexión con los nombres sigue abierta.

## Próximos pasos recomendados (orden de factibilidad)

1. **Resolver la conexión nombre↔registro de 16 bytes.** Es la pista más prometedora hasta ahora: hay texto legible y una tabla de tamaño fijo justo al lado. Probar otros anclajes para `f0`/`f1` (offset relativo al propio `linked_records`, índice directo en vez de offset en bytes, or relación inversa nombre→registro en vez de registro→nombre), y usar el crecimiento real de Bélgica (183 nombres + 201 registros nuevos) como caso de prueba dirigido — muchas menos entradas que comparar que en el resto del fichero.
2. **Aislar un cambio de una sola cámara conocida.** El ruido actual viene de que ~4 meses de cambios reales (posiblemente docenas de cámaras) están mezclados. Si se consigue una build con una ventana de tiempo más corta (p.ej. dos builds consecutivas separadas por semanas en vez de meses) el número de registros insertados sería menor y el diff más limpio.
3. **Diff binario del `.hafls`** (capa pan-europea) entre las dos builds con la misma metodología — mismo formato IFF, mismo tipo de análisis por regiones aplicable. Buscar también ahí la misma tabla de nombres/registros de 16 bytes.
4. **Vía exploit físico (`gen5w`)**: si en algún momento se obtiene acceso físico al HU, extraer `DecryptToPIPE` + `decryption_key.der` y descifrar `appnavi.tar` permitiría leer el parser real de HERE — la única vía que resuelve esto con certeza total en vez de por inferencia. Ver [`gen5w_exploit_ecosystem.md`](gen5w_exploit_ecosystem.md).
