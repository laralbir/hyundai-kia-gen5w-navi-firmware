...
## Continuación (misma sesión, tras pedir el usuario centrarse solo en renderizar el mapa, no en radares): la hipótesis `PackedTileId` también se probó — negativa

Se implementó el decodificador de `PackedTileId` completo (extracción de nivel desde los bits altos, número Morton, deinterleave a coordenadas de rejilla, esquina SO + tamaño del tile) y se escaneó buscando valores `u32` alineados a 4 bytes que decodifiquen a un `PackedTileId` **válido** (nivel 0-15, número Morton dentro de rango) cuyo bounding-box resultante solape España (35°-44,5°N, -10° a 4,5°E):

- **`.hafp03`** (5.000.000 palabras de 32 bits, niveles 3-11 — los niveles 12-15 se descartaron por ser casi siempre "válidos" con cualquier valor aleatorio, ver más abajo): **125 candidatos**. Prueba de permutación (10 controles con los mismos bytes barajados): **los 10 controles superan el resultado real** (rango 303-341, media 318 — el real es *más bajo* que cualquier control, no más alto). Negativo, y de forma inequívoca — no hay ni siquiera un indicio de enriquecimiento.
- **Árbol de cajas raíz de `.hafr`** (6.776 registros × 9 campos `u32` cada uno, offset `0x308`): **0 candidatos** en niveles 3-14 que solapen España.

**Nota metodológica importante encontrada al construir esta prueba:** los niveles 12-15 de `PackedTileId` son casi siempre "estructuralmente válidos" para cualquier valor de 32 bits aleatorio (la validación de rango se vuelve casi trivial a esos niveles — confirmado con un control de ruido uniforme puro, que da decenas de "candidatos válidos" por cada 500.000 palabras). Cualquier prueba futura de `PackedTileId` debe restringirse a niveles bajos/medios (3-11) o usar prueba de permutación — igual que con el Morton completo, un `PackedTileId` "que decodifica sin error" no es evidencia de nada por sí solo.

**Conclusión de esta sesión:** con el usuario pidiendo explícitamente centrarse en renderizar el mapa (no en cámaras), se probaron las dos hipótesis de geometría más fundamentadas del ecosistema NDS real (Morton completo y `PackedTileId` + rejilla) contra los ficheros más prometedores (`.hafp03`, `.hafr`) — **ambas dan negativo con rigor estadístico**. Esto agota, con un nivel de certeza alto, la vía de "encontrar geometría por inspección externa de bytes usando el algoritmo NDS real" tal como está planteada. La geometría real casi con toda seguridad requiere el parser real (`appnavi`, descifrado vía exploit físico `gen5w` — ver `docs/gen5w_exploit_ecosystem.md` y `tools/README.md` sección "Checklist del día D") para saber con certeza en qué offset/formato exacto vive, en vez de seguir infiriéndolo desde fuera.
